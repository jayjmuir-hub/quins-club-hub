...
-- proacl: {postgres=X/postgres}
-- Re-captured 4 Sep 2026 from db/migrations/20260904_claim_existing_player.sql:
-- a same-name row on the squad is CLAIMED rather than skipped.
...
-- #####################################################################
-- ##   ADDED 2026-09-04 — captured from the migration                ##
...
CREATE OR REPLACE FUNCTION public.senior_season_stats(_team uuid, _season text)
 RETURNS TABLE(player_id uuid, full_name text, games integer, starts integer, bench integer, tries integer, conversions integer, penalties integer, drops integer, yellows integer, reds integer)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
