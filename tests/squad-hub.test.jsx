...
describe('all-matches season record', () => {
  ...
  it('shows the W–D–L band for a scoring squad from scores on the hub', async () => {
    ...
    expect(within(band).getByTestId('stat-won')).toHaveTextContent('1')
    expect(within(band).getByTestId('stat-drawn')).toHaveTextContent('0')
    expect(within(band).getByTestId('stat-lost')).toHaveTextContent('0')
