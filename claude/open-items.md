...
| ~~`notice-push`~~ | Picked `push_subscriptions limit 1` as a poster; subscriber №1 is now a PARENT (36 subscribers, 2 admins). The runbook's "grows red as the club grows" class. Both picks now require role `admin`. 2 Sep 2026: the separate `v_two` (multi-squad) pick could also land on `v_poster` with no `distinct` excluded and no ORDER BY; it now excludes the poster and is ordered by `profile_id` for a deterministic choice |
