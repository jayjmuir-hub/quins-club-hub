...
- #729 — **Accounts: one compact Waiting for access note.** Same single-paragraph style as Players waiting (`PENDING_APPROVAL_NOTE`). Dismiss / revoke / login-survives stated once; the repeating footer is gone. Rot detector: `tests/accounts.test.jsx`.
- #728 — **Phone polish: dock caption gap, Hub tab air, Club Ops label.** Keep five dock tabs; smaller captions and a tighter Squad Hub pill so Schedule/Roster no longer collide at 360px. Shared `SquadHubNav` grey underline + 16px (`mb-4`) before the W–D–L band. Visible nav label Ops → Club Ops on the sidebar and account menu; route stays `/ops`. Rot detectors: `tests/nav.test.jsx`, `tests/squad-hub-nav.test.jsx`, `tests/sidebar-submenu.test.jsx`, `tests/app-shell.test.jsx`, `tests/account-menu.test.jsx`.
- #727 — **`20260917_chat_and_admin_counts` is APPLIED and live.**
- #726 — 🐢 **Chat at scale: unread count in SQL, scoped receipts, shared realtime channels.** From the 5 Sep performance review. The chat dot counted unread by fetching EVERY read receipt the caller could see (unfiltered, capped at 1000 rows by PostgREST, and the author arm hands a coach 300 rows for one post) — measured: one person at 300 rows off 197 messages — so it would have started lying within a season; now `count_unread_messages()`. The delivered tick upserted every unread id on every recount in every tab; now `mark_unread_delivered()` inserts only what is missing. The Admin badge read five tables; now `count_admin_waiting()`. `listMyMessageReads(selfId, ids)` asks only for my receipts on the rows on screen. One realtime channel per table shared by every subscriber (`src/data/subscribeToTable.js`), threads filtering the shared `messages` stream to their own rows, failing open on a DELETE. Load tickets in both thread hooks and `Chat.jsx` keyed by squad, so a slow response for the previous chat cannot land under the new header. ✅ Migration `20260917_chat_and_admin_counts` APPLIED to live 5 Sep 2026 (squash `b259ae1`), before the merge; the three functions confirmed on live with `authenticated` execute and `anon` refused. Rot detectors: `db/tests/chat-and-admin-counts.sql` (11 steps, rolled back on live, fault-injected), `tests/subscribe-to-table-shared.test.js`, `tests/messages-data.test.js`, `tests/count-admin-waiting.test.js`, `tests/data.test.js`.
- #725 — 🏉 **Club Ops hybrid C (play-up slice 3).** Home “Club Ops · N open” peek, `/ops` inbox (Open / Done), sidebar and account-menu **Ops**. Super admin Approve/Decline still `decide_playup_request`; head coach / age-group manager see their squads’ request status. `/admin/playups` redirects to `/ops`. No new migration. Rot detectors: `tests/club-ops-page.test.jsx`, `tests/club-ops.test.js`, `tests/scope.test.js`, `tests/sidebar-submenu.test.jsx`, `tests/account-menu.test.jsx`, `tests/nav.test.jsx`, `tests/app-shell.test.jsx`.
...
- #723 — 📎 **Chat FileCard: hide signed URLs, type pill, Download button.** Download is a control that fetch→blob→object URL (iOS WebKit `target=_blank` on the blob; installed PWA last-resorts to `window.open` of the blob). Never `<a href={signedUrl}>`. Shared `FileCard` on DMs, groups, channels, dock (`ChatBubble` / `MessageRow`), and welfare reports; `PendingFileChip` shares the type pill. Starred and chat-list stay text previews (`📄 File` / filename), not a second card. Rot detectors: `tests/chat-file-card.test.jsx`, `tests/download-blob.test.js`, `tests/chat-file-media.test.js`.
- #722 — 🏉 **Squad Hub pills: Call-ups and Play-ups.** Call-ups leave Overview (`/squad/:teamId/callups`); Senior Men see U18B only, Senior Women U18G only; empty-state **Not asked yet** until Ask. Play-ups Hub pill for head coach or age-group manager (`request_junior_playups` / `nominate_junior_playups`); assistant/medic/untagged refused. Shared `squadHubNavItems`. No migrations. Rot detectors: `tests/squad-hub-pills.test.jsx`, `tests/squad-playups.test.jsx`, `tests/callups.test.jsx`, `tests/squadHub.test.js`, `tests/sidebar-submenu.test.jsx`, `tests/gender.test.js`.
- #721 — 🏉 **Junior play-up request/nominate (slice 2).** Head coach or age-group manager files a host request or home nomination; assistant coach and medic are refused by `private.can_request_playup`. Queue `playup_requests` stays `requested` until a super admin Approves (`add_junior_playup`, parent consent still pending) or Declines. Thin inbox `/admin/playups` + Home peek. Migration `db/migrations/20260916_playup_requests.sql` not applied to live from this PR. Rot detectors: `tests/playup-request-sql.test.js`, `tests/playup-request-roster.test.jsx`, `tests/club-ops-page.test.jsx`, `tests/scope.test.js`, `tests/age-grade.test.js`, `db/tests/junior-playup-request.sql`. Also in this PR: ✅ `20260912_chat_file_attachments` / `20260913_junior_playup` / `20260914_junior_playup_consent` / `20260915_playup_staff_fix` / `20260915_feedback_thread` marked APPLIED (already on live 5 Sep 2026).
- #720 — 📝 **`20260915_feedback_thread` marked APPLIED.**
- #719 — 🏉 **The Seniors screen: one column, four rows, one card per squad.** Jay: "this senior section looks messy … things don't line up." This weekend, The pool, Season and Upcoming fixtures on the same three-card grid; the weekend's matches leave the fixtures list; W–D–L bars, league cards and stats cards fold into one Season card per squad; fixtures on a fixed-column grid so the squad tag no longer shifts the name; the switch beside the heading; "2 of 3 answered". **Full schedule carries the section** (`/schedule?team=section:senior_men`, selected on arrival, persisted). **Reports show when they came in**, and an open report counts on the Admin badge. **And a THREAD on every report** — Jay: "there is no way to send a follow-up message with the done, there is no thread of messages." `feedback_messages`: any number of messages from the reporter or an admin, on `/admin/needs-attention` and on the reporter's `/my-reports` and `?` sheet; an admin's message still becomes `admin_note` by trigger, which is what pushes the reporter; a reporter's last word puts the report back on the Admin badge. ✅ Migration `20260915_feedback_thread` APPLIED to live 5 Sep 2026 (squash `5b3dd18`), one existing reply backfilled, re-proved on the live schema with the harness rolled back, and the thread confirmed in the served bundle. Rot detectors: `tests/senior-section.test.jsx`, `tests/schedule.test.jsx`, `tests/feedback-triage.test.jsx`, `tests/my-reports.test.jsx`, `tests/admin-waiting-live.test.jsx`, `db/tests/feedback-thread.sql` (the report-count rule moved into SQL on 5 Sep 2026 and is now proved by `db/tests/chat-and-admin-counts.sql`).
- #718 — **fix(db): `playup_staff` aggregates uuid, not record.** `20260914` star-selected `SETOF uuid` from `approval_audience` inside `array_agg`, so apply failed (`COALESCE types record[] and uuid[] cannot be matched`). Same `CREATE OR REPLACE` in `db/migrations/20260915_playup_staff_fix.sql`. ✅ Migration `20260915_playup_staff_fix` APPLIED to live 5 Sep 2026 (squash `787d6a9`). Rot detector: `tests/playup-consent-sql.test.js`.
- #717 — 🏉 **Junior play-up parent consent (slice 1).** Guest twins stay `status=active`; `playup_consent` `pending`|`approved` on the guest row. Linked parents get a Home card + sheet + push; pending blocks match lineup only (`answer_junior_playup` + lineup trigger). ✅ Migration `20260914_junior_playup_consent` APPLIED to live 5 Sep 2026 (squash `bacd770`). Rot detectors: `tests/playup-consent.test.js`, `tests/playup-consent-banner.test.jsx`, `tests/playup-consent-sql.test.js`, `tests/lineup.test.jsx`, `tests/roster.test.jsx`.
...
- #713 — 🏉 **Junior play-up (second age group) via guest membership.** Super admin only. Home stays `players.team_id`; `add_junior_playup` / `remove_junior_playup` twin or drop active home memberships on another junior squad. Age groups block on the player sheet. ✅ Migration `20260913_junior_playup` APPLIED to live 5 Sep 2026 (squash `11df8e6`); `add_junior_playup` present. Rot detectors: `tests/playups.test.js`, `tests/age-groups-section.test.jsx`, `tests/players-list-membership.test.js`, `db/tests/junior-playup.sql`.
...
- #709 — 📎 **Chat file attachments (Excel, Word, PDF)** on `chat-media`. One allowlisted file per send (≤ 25 MB); FileCard on every chat surface; push preview is generic `📄 File` (never the original filename). Photo albums and voice unchanged. ✅ Migration `20260912_chat_file_attachments` APPLIED to live 5 Sep 2026 (squash `9860541`); chat-media has xlsx/pdf, 25MB. Ruling: `claude/decisions/2026-09-04-chat-file-attachments.md`.
...
- #707 — 📝 **`20260911_seniors_right` marked APPLIED** in the changelog and state-of-play.
- #706 — 🏉 **The `seniors` right — a named person reads BOTH senior sections.** Jay, 4 Sep: "the club captain for example, but might be others." An admin row carrying `seniors` reads both sections' rosters, availability, fixtures and season stats, no junior squad, no chat (outside `admin_team_reach` on purpose), and a minor's private row stays refused. `private.seniors_right_reach` as a fourth arm on `player read` / `event read` / `avail read` and both stats gates; ticked on `/admin/staff`; Roster and Schedule offer the senior squads to its holder. Harness steps 13–19 green on the rolled-back live run. ✅ Migration `20260911_seniors_right` APPLIED to live 4 Sep 2026 (squash `5eaa910`), re-proved on the live schema with the harness rolled back, and "Senior Sections" confirmed in the served bundle. Supersedes #705, whose ruling it carries.
...
- #703 — 📝 **Standings routes 2 and 3 deferred.** Jay, 4 Sep 2026: no AI reader for now and no paste-and-parse either; route 1 stands.
...
- #700 — 🔢 **The sidebar's Chat item wears the unread count.** Jay's ruling over the 23 Aug
  dot-not-a-number stance (recorded in `src/lib/useDockBadges.js`, which now returns
  `chatCount`); the phone dock keeps its dot. Zero on `/chat`. Rot detectors:
  `tests/sidebar-submenu.test.jsx`, `tests/dock-badges.test.jsx`.
