...
- #721 — 🏉 **Junior play-up request/nominate (slice 2).** Head coach or age-group manager files a host request or home nomination; assistant coach and medic are refused by `private.can_request_playup`. Queue `playup_requests` stays `requested` until a super admin Approves (`add_junior_playup`, parent consent still pending) or Declines. Thin inbox `/admin/playups` + Home peek. Migration `db/migrations/20260916_playup_requests.sql` not applied to live from this PR. Rot detectors: `tests/playup-request-sql.test.js`, `tests/playup-request-roster.test.jsx`, `tests/playup-request-inbox.test.jsx`, `tests/scope.test.js`, `tests/age-grade.test.js`, `db/tests/junior-playup-request.sql`.
