...
⚠️ **The live dock has moved on from this prototype snapshot** (`src/components/Nav.jsx`, `src/components/Sidebar.jsx`). The "More" tab retired 29 Aug 2026 (its home is the masthead account menu); the items are now **Home, Schedule, Roster, Chat**, with **Squad Hub** inserted before Chat for squad staff. And since 29 Aug 2026 **Home rides the CENTRE of the wide five-tab squad-staff dock, not the left edge** (Jay) — the app opens on Home (`start_url` `/` → Dashboard) and the middle of a full-width bar is the thumb's resting slot, so the tab you land on is the tab under your thumb. This is a **mobile-bar-only reorder**: `NAV_ITEMS` (and so the desktop Sidebar, a vertical list) keep Home first; only the horizontal dock moves it, at `floor(count/2)` — dead centre of the five-tab staff bar. ⚠️ **On the four-tab parent/player bar Home stays FAR LEFT** (Jay, 30 Aug 2026): that bar is a narrow centred **island** (fixed `w-[300px]`, since the full-width `justify-between` spread scattered only four tabs), and on the island the middle is no longer the thumb's rest, so Home keeps its natural NAV_ITEMS lead.
