...
⚠️ **The live dock has moved on from this prototype snapshot** (`src/components/Nav.jsx`, `src/components/Sidebar.jsx`). The "More" tab retired 29 Aug 2026 (its home is the masthead account menu); the items are now **Home, Schedule, Roster, Chat**, with **Squad Hub** inserted before Chat for squad staff. ✅ **Home sits at the FAR LEFT of the dock, in every view** (Jay, 31 Aug 2026) — its natural `NAV_ITEMS` slot, so the dock reorders nothing and the rendered order is the list order with Squad Hub spliced in before Chat. ⚠️ **This paragraph said the opposite from 29 to 31 Aug**: Home was centred at `floor(count/2)` on the wide five-tab staff bar, on the argument that the app opens on Home (`start_url` `/` → Dashboard) and the middle of a full-width bar is the thumb's resting slot. The four-tab parent/player bar was pulled back out of it within a day (#531), because on a narrow centred **island** (fixed `w-[300px]`, since the full-width `justify-between` spread scattered only four tabs) the middle is not where the thumb rests — and the resulting split put Home in two different places depending on who was looking at it. Reverted whole: one rule, all views. The reasoning, and the argument AGAINST reverting, are in `claude/decisions/2026-08-31-home-tab-left-in-all-views.md` — read it before proposing the centre again. `NAV_ITEMS` (and so the desktop Sidebar, a vertical list) led with Home throughout and was never affected.
