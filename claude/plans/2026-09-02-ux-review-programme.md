...
| Low | the two "Smaller items" lists and the extra findings | batches 1-2 building, `claude/ux-low-1`. SHIPPED. Spoken day labels + AddGameForm / TrainingPublish picker swap: `c314def` (#660). Share-outcome status, SquadHub dropdown, PlayerForm save hint and the extra-findings tap targets: `claude/ux-low-2`. **Not doing, and why:** avatar caption (the masthead is over-full 820-1280px; the menu's first row already says Settings); photo dimensions persisted at upload (a schema change for a shift the placeholder already covers); per-field error helpers and two-column event-form rows (polish, no report of harm); the faint-text and heading sweeps (a design-system pass, not a fix) |
