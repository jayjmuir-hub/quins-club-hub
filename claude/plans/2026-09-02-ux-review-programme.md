...
| Parent Medium | availability tap-to-clear + own child first; sign-up copy; notifications toggle copy; offline banner; Maps link; wizard draft | **SHIPPED** `9a6e39d` (#655). **Deferred:** avatar caption (masthead row over-full 820-1,280px) |
| Pattern leftovers | RequireAuth slow-load reload; Home first-load gate includes notices; photo placeholder while signing; PlayerDetail reserved space; Documents `window.confirm`; ChatPhoto Escape + focus; starred message scroll-to | **SHIPPED** `9929722` (#656). **Deferred:** photo dimensions at upload; per-field error helpers |
| Desktop | event-form sheet width | **SHIPPED** `dfb38ad` (#657) (fieldsets already present; two-column rows deferred) |
| Low | the two "Smaller items" lists and the extra findings | **SHIPPED** `938f2b6` (#658), `ab54bc4` (#663). Spoken day labels + AddGameForm / TrainingPublish picker swap: `c314def` (#660). Share-outcome status, SquadHub dropdown, PlayerForm save hint and the extra-findings tap targets: `ab54bc4` (#663). Player-import double tap and the screen-side raw errors: `06511e1` (#666); the data-layer error wrappers: `bfc2092` (#667). The seven small extra findings (cold-start heights, portal-card contrast, update notice, availability row saving, admin skeletons, invite email error placement, chat-bubble checkbox): `claude/ux-tail`. The Toast (design-system §4.24): `claude/toast`. **Not doing, and why:** avatar caption (the masthead is over-full 820-1280px; the menu's first row already says Settings); photo dimensions persisted at upload (a schema change for a shift the placeholder already covers); per-field error helpers and two-column event-form rows (polish, no report of harm); the faint-text and heading sweeps (a design-system pass, not a fix) |
