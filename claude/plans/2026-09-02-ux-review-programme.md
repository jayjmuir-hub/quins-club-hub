...
| Low | the two "Smaller items" lists and the extra findings | **SHIPPED** `938f2b6` (#658), `ab54bc4` (#663). Spoken day labels + AddGameForm / TrainingPublish picker swap: `c314def` (#660). Share-outcome status, SquadHub dropdown, PlayerForm save hint and the extra-findings tap targets: `ab54bc4` (#663). Player-import double tap and the screen-side raw errors: `claude/ux-errors`; the data-layer error wrappers: Training Sessions session. **Not doing, and why:** avatar caption (the masthead is over-full 820-1280px; the menu's first row already says Settings); photo dimensions persisted at upload (a schema change for a shift the placeholder already covers); per-field error helpers and two-column event-form rows (polish, no report of harm); the faint-text and heading sweeps (a design-system pass, not a fix) |
