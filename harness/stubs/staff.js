...
// ⚠️ BOTH SQUADS NOW READ THEIR STAFF FROM THE MAP, AND `t2` USED TO BE HARD-CODED
// EMPTY. That was true until 16 Aug 2026, when `t2` gained the two-person squad
// above — after which this function said `t2` had nobody while Home showed two
// people in it, from the same stub file. The empty-squad case this list exists to
// carry is now `t3`, which is empty because nothing puts anyone in it rather than
// because a literal says so.
export async function listSquadStaff() {
  const staff = await listMySquadStaff()
  return [
    { id: 't1', name: 'U12 Boys', staff: staff.get('t1') ?? [] },
    { id: 't2', name: 'U14 Boys', staff: staff.get('t2') ?? [] },
    { id: 't3', name: 'U16 Boys', staff: staff.get('t3') ?? [] },
  ]
}
